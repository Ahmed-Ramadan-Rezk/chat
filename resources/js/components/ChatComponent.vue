        ...
        <div ref="messageContainer" class="overflow-y-auto p-4 mt-3 flex-grow border-t border-gray-200">
            <div class="space-y-4">
                <div v-for="message in messages" :key="message.id"
                    :class="{ 'text-right': message.sender_id === currentUser }" class="mb-4">
                    <div :class="message.sender_id === currentUser ? 'bg-gray-200 text-gray-800' : 'bg-indigo-500 text-dark'"
        ...
        <div class="border-t pt-4">
            <form @submit.prevent="sendMessage">
                <div class="flex items-center">
                    ...
                    <button type="submit"
                        class="ml-2 bg-indigo-500 text-dark p-3 rounded-lg shadow hover:bg-indigo-600 transition duration-300 flex items-center justify-center">
                        <i class="fas fa-paper-plane"></i>
                        <span class="ml-2 text-dark">Send</span>
                    </button>
                </div>
            </form>
        </div>
...
const fetchMessages = async () => {
    try {
        const response = await axios.get(`http://chat.test/messages/${props.user.id}`);
...
const sendMessage = async () => {
    if (newMessage.value.trim() !== '') {
        try {
            const response = await axios.post(`http://chat.test/messages/${props.user.id}`, {
...
onMounted(() => {
    ...
    Echo.private(`chat.${props.currentUser}`)
